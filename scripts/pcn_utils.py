...
def merge_dicts(dicts):
    ...
    res = {}
    for i, current in enumerate(dicts):
        if i == 0:
...
def compute_dihe(xyz, indices):
    ...
    b1 = xyz[ix10[:, 1]] - xyz[ix10[:, 0]] 
    b2 = xyz[ix21[:, 1]] - xyz[ix21[:, 0]] 
    b3 = xyz[ix32[:, 1]] - xyz[ix32[:, 0]] 

    c1 = torch.cross(b2, b3)
    c2 = torch.cross(b1, b2)

    p1 = (b1 * c1).sum(-1)
    p1 *= ((b2 * b2).sum(-1) + EPS)** 0.5
    p2 = (c1 * c2).sum(-1)

    return torch.arctan(p1 / (p2 + EPS))
...
def loop(loader, optimizer, device, model, beta, epoch, 
    ...
    for i, batch in enumerate(loader):
        ...
        gen_dihe = compute_dihe(xyz_recon, batch['dihe_idxs'])
        data_dihe = compute_dihe(xyz, batch['dihe_idxs']).to(xyz_recon.device)
        loss_dihe = (gen_dihe - data_dihe).pow(2).mean()
        ...
        if loss.item() >= gamma * 300.0 or torch.isnan(loss):
            print(loss.item())
        ...
        dihe_loss.append(loss_dihe.item())
        ...
        mean_dihe = np.array(dihe_loss).mean()
...
def get_all_true_reconstructed_structures(loader, device, model, atomic_nums=None, n_cg=10, atomwise_z=False, tqdm_flag=True, reflection=False):
    ...
    for batch in loader:
        ...
        cg_xyzs.append(batch['ca_xyz'].detach().cpu())

        recon  = xyz_recon.detach().cpu()
        data = xyz.detach().cpu()

        recon_x_split =  torch.split(recon, batch['num_atoms'])
        data_x_split =  torch.split(data, batch['num_atoms'])

        atomic_nums_split = torch.split(atomic_nums, batch['num_atoms'])
